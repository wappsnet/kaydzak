...
function runApp(): void
{
    ...
    if (isset($wpData["supports"])) {
        foreach ($wpData["supports"] as $value) {
            if (isset($value['params'])) {
                add_theme_support($value['name'], $value['params']);
            } else {
                add_theme_support($value['name']);
            }
        }
    }
    ...
    if (isset($wpData["features"])) {
        foreach ($wpData["features"] as $feature) {
            if (isset($feature['params'])) {
                add_theme_support($feature['name'], $feature['params']);
            } else {
    ...
    if (isset($wpData["menus"])) {
        if (is_array($wpData["menus"])) {
            foreach ($wpData["menus"] as $menu) {
                $id = wp_create_nav_menu($menu['name']);
                if (!is_wp_error($id)) {
                    wp_update_nav_menu_object($id, $menu['data']);
                }
            }
        }
    }

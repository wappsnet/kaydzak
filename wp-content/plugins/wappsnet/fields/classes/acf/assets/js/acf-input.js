( function( window, undefined ) {
	...
	/**
	 * Handles managing all events for whatever you plug it into. Priorities for acf_hooks are based on lowest to highest in
	 * that, lowest priority acf_hooks are fired first.
	 */
	var EventManager = function() {
		...
		/**
		 * Contains the acf_hooks that get registered with this EventManager. The array for storage utilizes a "flat"
		...
		function _addHook( type, hook, callback, priority, context ) {
			...
			var acf_hooks = STORAGE[ type ][ hook ];
			if( acf_hooks ) {
				acf_hooks.push( hookObject );
				acf_hooks = _hookInsertSort( acf_hooks );
			}
			else {
				acf_hooks = [ hookObject ];
			}

			STORAGE[ type ][ hook ] = acf_hooks;
		}

		/**
		 * Use an insert sort for keeping our acf_hooks organized based on priority. This function is ridiculously faster
		 * than bubble sort, etc: http://jsperf.com/javascript-sort
		 *
		 * @param acf_hooks The custom array containing all of the appropriate acf_hooks to perform an insert sort on.
		 * @private
		 */
		function _hookInsertSort( acf_hooks ) {
			var tmpHook, j, prevHook;
			for( var i = 1, len = acf_hooks.length; i < len; i++ ) {
				tmpHook = acf_hooks[ i ];
				j = i;
				while( ( prevHook = acf_hooks[ j - 1 ] ) &&  prevHook.priority > tmpHook.priority ) {
					acf_hooks[ j ] = acf_hooks[ j - 1 ];
					--j;
				}
				acf_hooks[ j ] = tmpHook;
			}
	...
	window.wp = window.wp || {};
	window.wp.acf_hooks = new EventManager();
...
(function($){
	...
	*/
	...
	acf = {
		...
		*  add_action
		*
		*  This function uses wp.acf_hooks to mimics WP add_action
		*
		*  @type	function
		*  @date	8/09/2014
		*  @since	5.0.0
		...
		*/
		...
		add_action: function() {
			...
			for( k in actions ) {
				...
				wp.acf_hooks.addAction.apply(this, arguments);
			}
		...
		*  remove_action
		*
		*  This function uses wp.acf_hooks to mimics WP remove_action
		*
		*  @type	function
		*  @date	8/09/2014
		*  @since	5.0.0
		...
		*/
		...
		remove_action: function() {
			...
			wp.acf_hooks.removeAction.apply(this, arguments);
		...
		*  do_action
		*
		*  This function uses wp.acf_hooks to mimics WP do_action
		*
		*  @type	function
		*  @date	8/09/2014
		*  @since	5.0.0
		...
		*/
		...
		do_action: function() {
			...
			wp.acf_hooks.doAction.apply(this, arguments);
		...
		*  add_filter
		*
		*  This function uses wp.acf_hooks to mimics WP add_filter
		*
		*  @type	function
		*  @date	8/09/2014
		*  @since	5.0.0
		...
		*/
		...
		add_filter: function() {
			...
			wp.acf_hooks.addFilter.apply(this, arguments);
		...
		*  remove_filter
		*
		*  This function uses wp.acf_hooks to mimics WP remove_filter
		*
		*  @type	function
		*  @date	8/09/2014
		*  @since	5.0.0
		...
		*/
		...
		remove_filter: function() {
			...
			wp.acf_hooks.removeFilter.apply(this, arguments);
		...
		*  apply_filters
		*
		*  This function uses wp.acf_hooks to mimics WP apply_filters
		*
		*  @type	function
		*  @date	8/09/2014
		*  @since	5.0.0
		...
		*/
		...
		apply_filters: function() {
			...
			return wp.acf_hooks.applyFilters.apply(this, arguments);
